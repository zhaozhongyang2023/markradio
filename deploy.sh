...
# 2. 停止服务
echo "[2/4] 停止远端服务..."
ssh "${SSH_OPTS[@]}" "$PI_USER@$PI_HOST" "cd $PI_DIR && bash scripts/moodwave.sh stop" 2>/dev/null || echo "  (服务可能未运行)"
...
rsync -avz \
  -e "$RSYNC_SSH" \
  "$PROJECT_DIR/scripts/moodwave.sh" \
...
ssh "${SSH_OPTS[@]}" "$PI_USER@$PI_HOST" "cd $PI_DIR && chmod +x scripts/moodwave.sh && bash scripts/moodwave.sh $REMOTE_MODE"
