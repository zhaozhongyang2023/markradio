...
**不用切出游戏**，Decky 插件里一键开启。注入专属 **DJ 灵魂**——克制、少语、有留白。

| 氛围 | AI DJ 会这样说 | 适合 |
|:--|:--|:--|
| 🗡️ Boss 战 | "今晚燃一点。" | 老头环、只狼、巫师3 战斗 |
| 🗺️ 探索地图 | "慢慢走。" | 巫师3 威伦骑马、塞尔达旷野 |
| 🌙 雨夜跑图 | "外面下雨，慢慢来。" | 艾尔登法环雨夜、JRPG 长途跋涉 |
| 🏎️ 赛车竞速 | "速度别停。" | 马车8、Forza |
| 🌾 种田放松 | "别太累了。" | 星露谷、牧场物语 |
| 📺 模拟器怀旧 | "像小时候一样。" | GBA、PSP 模拟器 |

> V15 新特性：输入游戏名（如「巫师3」），AI 自动感知**游戏世界氛围**——中世纪篝火、赛博霓虹、JRPG 旅程感。
...
| 平台 | 模式 | 说明 |
|------|------|------|
| 🎮 Steam Deck | 桌面 PWA + 游戏模式 Decky 插件 | 主力平台，插件轻量品牌化 + 极简模式 |
...
POST /api/ai/game-radio    # 游戏电台（注入 DJ 灵魂 + 游戏世界感）
POST /api/play             # 播放
POST /api/pause            # 暂停
POST /api/next             # 下一首
POST /api/prev             # 上一首
