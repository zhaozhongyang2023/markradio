...
const text = process.env.MARKRADIO_TTS_TEXT || '';
const reference = process.env.MARKRADIO_TTS_REFERENCE || '';
const output = process.env.MARKRADIO_TTS_OUTPUT || '';
...
const cli = process.env.MARKRADIO_F5_TTS_CLI || resolve(process.cwd(), '.venv-tts/bin/f5-tts_infer-cli');
const commandEnv = {
  ...process.env,
  PATH: ['/usr/local/bin', '/opt/homebrew/bin', process.env.PATH || ''].filter(Boolean).join(':')
};

if (!text.trim()) fail('MARKRADIO_TTS_TEXT 为空');
if (!reference || !existsSync(reference)) fail(`参考音频不存在：${reference}`);
if (!output) fail('MARKRADIO_TTS_OUTPUT 为空');
